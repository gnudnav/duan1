...
public class SanPham_NamDao{
    private DbHelper dbHelper;
    public SanPham_NamDao(Context context){
        dbHelper=new DbHelper(context);
    }

    public ArrayList<SanPham> getDsSanPhamNam(){
        ArrayList<SanPham>list=new ArrayList<>();
        SQLiteDatabase sqLiteDatabase=dbHelper.getReadableDatabase();
        Cursor cursor=sqLiteDatabase.rawQuery("SELECT * FROM SANPHAM WHERE maloai=1",null);
